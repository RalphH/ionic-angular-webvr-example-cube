...
export class SpinningCubeComponent implements OnInit {
    ...
    private cubeZ: number = 0;
    private cubeMoveX: number = 0.001;
    private cubeMoveY: number = 0.001;
    private cubeMoveInterval: number = -100;
    private cubeBoundTop: number = 0;
    private cubeBoundLeft: number = 0;
    private cubeBoundRight: number = 2;
    private cubeBoundBottom: number = 2;
	...
	ngOnInit() {
        ...
        this.cube = this.createCube(0.30, new THREE.Color('rgb(0,255,0)'));
        this.cubeX = 0;
        this.cubeY = this.controls.userHeight;
        this.cubeZ = -0.8;
        this.cubeBoundTop = 1;
        this.cubeBoundLeft = -0.5;
        this.cubeBoundRight = 0.5;
        this.cubeBoundBottom = 2;
        this.cube.position.set(this.cubeX, this.cubeY, this.cubeZ);
    ...
    bounceCubeInterval: number = window.setInterval( () => {
        this.cubeX = this.cubeX + this.cubeMoveX;
        this.cubeY = this.cubeY + this.cubeMoveY;

        if(this.cubeX > this.cubeBoundRight) {
            this.cubeX = this.cubeBoundRight;
            this.cubeMoveX = -this.cubeMoveX;  
            console.log("bounds right", this.cubeX);          
          }
        if(this.cubeY > this.cubeBoundBottom) {
            this.cubeY = this.cubeBoundBottom;
            this.cubeMoveY = -this.cubeMoveY;   
            console.log("bounds bottom", this.cubeY);         
        }
        if(this.cubeX < this.cubeBoundLeft) {
            this.cubeX = this.cubeBoundLeft;
            this.cubeMoveX = -this.cubeMoveX; 
            console.log("bounds left", this.cubeX);           
        }
        if(this.cubeY < this.cubeBoundTop) {
            this.cubeY = this.cubeBoundTop;
            this.cubeMoveY = -this.cubeMoveY; 
            console.log("bounds top", this.cubeY);           
        }
        
        this.cube.position.set(this.cubeX, this.cubeY, this.cubeZ);
    }, this.cubeMoveInterval);    
}
